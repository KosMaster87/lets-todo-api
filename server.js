...
// app.use(
//   cors({
//     origin: ["https://app-restful-notes-user-session.dev2k.org"], // <--- Frontend-URL (must include protocol)
...
// Middleware: Pool-Auswahl based on userId or guestId
app.use(async (req, res, next) => {
  // falls ein User eingeloggt…
  if (req.cookies.userId) {
    const [rows] = await userPool.query(
      `SELECT db_name FROM users WHERE id = ?`, [req.cookies.userId]
    );
    if (rows.length) {
      const dbName = rows[0].db_name;
      if (!guestPools[req.cookies.userId]) {
        guestPools[req.cookies.userId] = corePool; // oder eigener Pool zur User-DB
      }
      req.pool = guestPools[req.cookies.userId];
      return next();
    }
  ...
  // andernfalls via sessionRouter bereits req.pool gesetzt
  if (req.pool) return next();
  res.status(500).json({ error: "Keine Session initialisiert" });
});
...
app.get("/api/todos/:id", /* … wie gehabt … */);
app.post("/api/todos", /* … */);
